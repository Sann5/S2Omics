...
def label_broadcasting(WSI_datapath, WSI_save_folder, 
                      SO_datapath, SO_save_folder,
    ...
    if not os.path.exists(SO_save_folder):
        os.makedirs(SO_save_folder)
    SO_save_folder = SO_save_folder+'/'
    ...
    annotation_file = pd.read_csv(SO_datapath+'annotation_file.csv')
    ...
    if len(SO_cache_path) > 0:
        SO_cache_path = SO_cache_path
    else:
        SO_cache_path = SO_pickle_folder
    SO_he_embed_total = []
    i = 0
    while 1 > 0:
        if os.path.exists(SO_cache_path+foundation_model+f'_embeddings_downsamp_1_part_{i}.pickle'):
            SO_he_embed_part = load_pickle(SO_cache_path+foundation_model+f'_embeddings_downsamp_1_part_{i}.pickle')
    ...
    if not os.path.exists(WSI_save_folder):
        os.makedirs(WSI_save_folder)
    WSI_save_folder = WSI_save_folder+'/'
    ...
    if len(WSI_cache_path) > 0:
        WSI_cache_path = WSI_cache_path
    else:
        WSI_cache_path = WSI_pickle_folder
    WSI_he_embed_total = []
    i = 0
    while 1 > 0:
        if os.path.exists(WSI_cache_path+foundation_model+f'_embeddings_downsamp_1_part_{i}.pickle'):
            WSI_he_embed_part = load_pickle(WSI_cache_path+foundation_model+f'_embeddings_downsamp_1_part_{i}.pickle')
