...
def region_selection_random(save_folder, cluster_image, cluster_image_rgb, valid_mask, num_clusters,
    ...
    while if_stop is False:
        # find the best (curr_num_roi+1)^th ROI
        best_roi, best_roi_mask, best_comp, best_roi_score = region_selection_few(
            cluster_image, valid_mask, num_clusters, window_size, target_proportion,
            pre_best_roi, pre_best_roi_mask, pre_best_comp, 1, fusion_weights,
        ...
        best_roi, best_roi_mask, best_comp, best_roi_score = region_selection_few(
            cluster_image, valid_mask, num_clusters, window_size, target_proportion,
            pre_best_roi, pre_best_roi_mask, pre_best_comp, 2, fusion_weights,
            num_samp_per_iter, samp_step)
...
def roi_selection_for_multiple_sections(save_folder_list,
                                        ...
                                        prior_preference=1):
    '''
    select best circle ROI(s) jointly across multiple sections.
    ...
    Parameters:
        ...
        has_annotation: if True, use the annotation file instead of histology segmentation, default=False
    ...
    roi_subdir = p5_root + f'circle_roi_size_{roi_size[0]}_{roi_size[1]}/'
    save_subfolder = roi_subdir + f'prior_preference_{prior_preference}/'
    os.makedirs(save_subfolder, exist_ok=True)

    completion_marker = p5_root + 'multi_circle_roi_selection_complete.pickle'
    expected_metadata = {
    ...
    if stage_is_complete(completion_marker, expected_metadata, required_outputs=(best_roi_path,)):
        print(
            'Skipping multi-section circle ROI selection; outputs already exist for '
    ...
    concat_image_shape = [np.sum(image_shape_list, axis=0)[0] + (n_images - 1) * padding_size,
                          np.max(image_shape_list, axis=0)[1]]
    ...
    if dpi * length > np.power(2, 16):
        ...
        plt_figsize = ((concat_image_shape[1] * down_samp_step * reduce_ratio) // 100,
                       (concat_image_shape[0] * down_samp_step * reduce_ratio) // 100)
    if plt_figsize[0] < 10:
